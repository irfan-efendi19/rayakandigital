...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div>
            ...
            <a href="<?= SITE_UNDANGAN ?>/<?= $order[0]->domain ?>" target="_blank" class="btn btn-sm">Lihat Website</a>
        ...
        <div class="col-xl-12 col-lg-12 mb-4">
            <div class="card">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    ...
                    <h6 class="m-0 font-weight-bold " style="color:red"><a class="btn btn-warning"
                            href="<?= base_url('user/invoice') ?>">Invoice</a> : Selesaikan pembayaran anda sebelum
                        <?= $tglExpFormated ?> untuk menikmati fiturnya|| Status Undangan :
                        <?php if($today < $tglExp) { echo "Trial"; }else{ echo "Tidak Aktif";}?></h6>

                    <?php }else if($pembayaran[0]->status == 1 && $metode_bayar == 'manual'){ ?>
                    <h6 class="m-0 font-weight-bold " style="color:orange"><a class="btn btn-warning"
                            href="<?= base_url('user/invoice') ?>">Invoice</a> : Pembayaran anda menunggu dikonfirmasi
                        || Status Undangan : <?php if($today < $tglExp) { echo "Trial"; }else{ echo "Tidak Aktif";}?>
                    </h6>
                    <?php }else if($pembayaran[0]->status == 1 && $metode_bayar != 'manual'){ ?>
                    <h6 class="m-0 font-weight-bold " style="color:orange"><a class="btn btn-warning"
                            href="<?= base_url('user/invoice') ?>">Invoice</a> : Selesaikan pembayaran anda sebelum
                        <?= $expiry_date ?> || Status Undangan :
                        <?php if($today < $tglExp) { echo "Trial"; }else{ echo "Tidak Aktif";}?></h6>
                    <?php } else if($pembayaran[0]->status == 2 && $today >= $tglNonaktif ){ 
                    ?>
                    <h6 class="m-0 font-weight-bold " style="color:red"><a class="btn btn-warning"
                            href="<?= base_url('user/invoice') ?>">Invoice</a> : Masa Aktif Undangan sudah habis pada
                        tanggal <?= $tglNonaktifFormated ?>, Silahkan lakukan pembayaran lagi untuk memperpanjang masa
                        aktif undangan </h6>
                    <?php }else{ ?>
                    ...
                    <div class="customer-message align-items-center">
                        <a class="font-weight-bold" href="#">
                            ...
                            <div class="small text-gray-500 message-time font-weight-bold">
                                <?= \esc($row->nama_komentar) ?></div>
                        </a>
                    </div>
                    <?php } ?>

                    <div class="card-footer text-center">
                        <a class="m-0 small text-primary card-link" href="<?= base_url('user/ucapan') ?>">Lihat Lebih <i
